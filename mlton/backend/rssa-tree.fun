 ...
 * Copyright 2013 Matthew Fluet, David Larsen.
 * Copyright (C) 2009.
   ...
   struct
      ...
      fun layoutHeader (T {entries, name, raises, returns, ...}): Layout.t =
         let
            open Layout
         in
            align (seq [str "fun ", Func.layout name,
                        if !Control.showTypes
                           then seq [str ": ",
                                     record [("raises",
                                              Option.layout
                   ...
                   Vector.toListMap (entries, FunctionEntry.layoutHeaders))
         end
      ...
      fun foreachDef (T {entries, blocks, ...}, f) =
      ...
      fun dominatorForest (t as T {entries, blocks, ...}): Block.t Tree.t vector =
         let
      ...
      fun loopForest (t as T {entries, blocks, ...}, predicate) =
         let
            val (g, labelNode, nodeInfo) = overlayGraph t
            val _ =
               Vector.foreach
                ...
                (transfer, fn to =>
                 if predicate (from, (nodeInfo o labelNode) to)
                    then ignore (Graph.addEdge (g, {from = labelNode label, to = labelNode to}))
                         ...
                         (entries, fn FunctionEntry.T {start, ...} =>
                          ignore (Graph.addEdge (g, {from = labelNode start, to = labelNode to})))))
            val roots = Vector.map (entries, labelNode o FunctionEntry.start)
         in
            Graph.loopForestSteensgaard (g, {roots = roots, nodeValue = nodeInfo})
         end
      ...
      fun shuffle (f: t): t =
         let
            val {blocks, entries, name, raises, returns} = dest f
            val entries = Array.fromVector entries
            val () = Array.shuffle entries
            val blocks = Array.fromVector blocks
            val () = Array.shuffle blocks
         in
            new {blocks = Array.toVector blocks,
                 entries = Array.toVector entries,
                 name = name,
                 raises = raises,
                 returns = returns}
         end
            ...
            fun loopFunction (f: Function.t): Function.t =
               let
                  val {name, entries, raises, returns, ...} =
                     Function.dest f
                  val () =
                     Vector.foreach (entries, loopFormals o FunctionEntry.args)
                  val blocks = ref []
                  val () =
                     Function.dfs
         ...
         let
            ...
            fun loopFunc (f: Function.t, isMain: bool): unit =
               let
                  ...
                  val {entries, blocks, ...} = Function.dest f
                  val _ =
                     Vector.foreach
                  ...
                  (* Descend the dominator forest, verifying that variable
