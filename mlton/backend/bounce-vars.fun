...
fun transformFunc func =
   let
      val {entries, blocks, name, raises, returns} = Function.dest func
      ...
      val _ = Vector.foreach (entries, fn FunctionEntry.T {args, ...} => Vector.foreach (args,
         fn (x, _) =>
            case varInfo x of
   ...
   in
      Function.new
         {entries=entries, blocks=newBlocks,
