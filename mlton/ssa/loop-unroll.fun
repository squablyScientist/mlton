              ...
              Transfer.Call {args, entry, func, return} =>
                let
                  val newReturn =
                    ...
                    | _ => return
                in
                  Transfer.Call {args = args, entry = entry, func = func, return = newReturn}
                end
            | Transfer.Case {cases, default, test} =>
                let
...
fun setDoms forest =
  let
    ...
    fun loopForest (forest, doms) =
       Vector.foreach (forest, fn tree => loopTree (tree, doms))
    and loopTree (tree, doms) =
      case tree of
        Tree.T (block, children) =>
          (setDomInfo (Block.label block, doms) ;
           loopForest (children, (Block.label block)::doms))
    val () = loopForest (forest, [])
  in
    (domInfo, destroy)
  end
...
fun optimizeFunction loadGlobal function =
   let
      ...
      val forest = Graph.loopForestSteensgaard(graph, {roots = roots, nodeValue = fn x => x})
      val dforest = Function.dominatorForest function
      val (domInfo, destroy) = setDoms dforest
