   ...
   fun transform (Program.T {datatypes, globals, functions, main}): Program.t =
      let
         ...
         fun loopTransfer transfer =
            case transfer of
               ...
               | Transfer.Call {args, entry, func, return} =>
                    Transfer.Call {args=freshenVec args, entry=entry, func=func, return=return}
         ...
         fun loopFunction func =
            let
               val {blocks, entries, mayInline, name, raises, returns} = Function.dest func
               val newBlocks = Vector.map(blocks, loopBlock)
            in
               Function.new {entries=entries, blocks=newBlocks, mayInline=mayInline, name=name,
