      ...
      val parse =
         let
            ...
            val parseCall =
               Func.parse >>= (fn func =>
               sym "@" *>
               FuncEntry.parse >>= (fn entry =>
               parseArgs >>= (fn args =>
               pure (fn return => pure {func = func, entry = entry, args = args, return = return}))))
         in
            any
            [Bug <$ kw "bug",
             Call <$>
...
structure FunctionEntry =
   struct
      ...
      fun layoutHeader (T {args, name, start, ...}) =
         let
            open Layout
         in
            mayAlign [seq [str "entry ",
      ...
      val parse =
         ...
         in
            kw "entry" *>
            FuncEntry.parse >>= (fn name =>
            parseFormals >>= (fn args =>
            sym "=" *>
            Label.parse >>= (fn start =>
            paren (pure ()) *>
            pure (T {name = name, args = args, start = start}))))
         end
   end
...
structure Function =
   struct
      ...
      in
         ...
         fun layoutDot (f, layoutVar) =
            let
               ...
               val {name, entries, blocks, returns, raises, ...} = dest f
               open Dot
               val graph = Graph.new ()
               val {get = nodeOptions, ...} =
               ...
               val entryNodes =
                  ...
                  (entries, fn FunctionEntry.T {name, args, start} =>
                   let
                      val entryNode = entryNode name
                      val _ = edge (entryNode, labelNode start, "Start", Solid)
                      val lab = [(toStringTransfer (Transfer.Goto {dst = start, args = Vector.new0 ()}), Left)]
                      val lab =
                         (toStringHeader ("entry " ^ FuncEntry.toString name, args), Left)::
                         lab
                      val _ = setNodeText (entryNode, lab)
                   in
                      entryNode
                   end)
               val funNode =
                  let
                     ...
                     val _ =
                        Vector.foreach
                        (entryNodes, fn entryNode =>
                         edge (funNode, entryNode, "entry", Solid))
                     val lab = []
                     val lab =
                        if !Control.showTypes
                           then ((Layout.toString o Layout.record)
                                 [("returns",
                                   Option.layout
            ...
            in
               {destroy = destroyEntryNode o destroyLabelNode,
                controlFlowGraph = controlFlowGraphLayout,
                dominatorForest = dominatorForestLayout,
      ...
      fun layoutHeader (f: t): Layout.t =
         let
            val {entries, name, mayInline, raises, returns, ...} = dest f
            open Layout
            val (sep, rty) =
               if !Control.showTypes
                  then (str ":",
                        indent (record [("returns",
                                         Option.layout
                                         (Vector.layout Type.layout)
                                         returns),
                                        ("raises",
         ...
         in
            align ((mayAlign [seq [str "fun ",
                                   if mayInline then empty else str "noinline ",
                                   Func.layout name,
                                   sep],
                              rty])
                   :: Vector.toListMap (entries, fn entry =>
                                        indent (FunctionEntry.layoutHeader entry, 2)))
         end
      ...
      val parseHeader =
         ...
         in
            ...
            many FunctionEntry.parse >>= (fn entries =>
            pure (Option.isNone noInline, name, returns, raises, Vector.fromList entries)))))
         end
      ...
      val parse =
         ...
         in
            new <$>
            (parseHeader >>= (fn (mayInline, name, returns, raises, entries) =>
         ...
         let
            ...
            val _ =
               if not (!Control.keepDot)
                  ...
                  let
                     val {destroy, controlFlowGraph, dominatorForest, loopForest} =
                     ...
                     val _ = doit ("dom", dominatorForest ())
         ...
         let
            ...
            val parseProgram =
               ...
               (many Datatype.parse >>= (fn datatypes =>
                ...
                Func.parse >>= (fn mainFunc =>
                sym "@" *>
                FuncEntry.parse >>= (fn mainEntry =>
