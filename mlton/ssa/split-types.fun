   ...
   let
      ...
      val functions =
         List.map (functions, fn f =>
            let
               val { entries, blocks, mayInline, name, ... } =
                  Function.dest f
               val { raises = raiseTys, returns = returnTys } = func name
               val entries =
                  Vector.map (entries, fn FunctionEntry.T { args, name, start } =>
                    FunctionEntry.T
                    { args = Vector.map2 (args, entry name, fn ((v, _), typeInfo) => (v, getTy typeInfo)),
                      name = name,
                      start = start })
            in
               Function.new
